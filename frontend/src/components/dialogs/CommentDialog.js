...
class CommentDialog extends Component {
  ...
  setComment = () => {
    ...
    backend
      .post(`/api/connections/${this.props.connectionId}/comment`, {comment})
      .then(() => this.close())
      .catch((e) => {
        log.error(e);
  ...
  render() {
    return (
      <Modal show size="md" aria-labelledby="comment-dialog" centered>
        <Modal.Header>
          <Modal.Title id="comment-dialog">~/.comment</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <TextField value={this.state.comment} onChange={(comment) => this.setState({comment})} rows={7} error={this.state.error} />
        </Modal.Body>
