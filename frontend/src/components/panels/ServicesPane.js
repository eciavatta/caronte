  ...
  render() {
    ...
    let services = this.state.services.map((s) => (
      ...
      >
        ...
        <td>{s['name']}</td>
        <td>
          <ButtonField name={s['color']} color={s['color']} small />
        </td>
        <td>{s['notes']}</td>
      </tr>
    ));
      ...
      <div className="pane-container service-pane">
          ...
          <div className="section-header">
            ...
            {this.state.servicesStatusCode && (
              <span className="api-response">
                <LinkPopover text={this.state.servicesStatusCode} content={this.state.servicesResponse} placement="left" />
              </span>
            )}
          </div>
        ...
        <div className="pane-section service-edit">
          <div className="section-header">
            ...
            <span className="api-response">
              <LinkPopover text={this.state.serviceStatusCode} content={this.state.serviceResponse} placement="left" />
            </span>
          </div>
              ...
              <Row>
                <Col>
                  <NumericField
                    ...
                    min={0}
                    max={65565}
                    error={this.state.servicePortError}
                  />
                  <InputField name="name" value={service.name} onChange={(v) => this.updateParam((s) => (s.name = v))} error={this.state.serviceNameError} />
                  <ColorField value={service.color} error={this.state.serviceColorError} onChange={(v) => this.updateParam((s) => (s.color = v))} />
                </Col>

                <Col>
                  <TextField name="notes" rows={7} value={service.notes} onChange={(v) => this.updateParam((s) => (s.notes = v))} />
                </Col>
              </Row>
            </Container>
          ...
          <div className="section-footer">
            ...
            {isUpdate && <ButtonField variant="red" name="delete_service" bordered onClick={this.deleteService} />}
            <ButtonField variant={isUpdate ? 'blue' : 'green'} name={isUpdate ? 'update_service' : 'add_service'} bordered onClick={this.updateService} />
          </div>
        </div>
      </div>
    );
