...
class PcapsPane extends Component {
  ...
  uploadPcap = () => {
    ...
    backend
      .postFile('/api/pcap/upload', formData)
      .then((res) => {
        this.setState({
  ...
  render() {
    let sessions = this.state.sessions.map((s) => {
      ...
      return (
        <tr key={s.id} className="row-small row-clickable">
          <td>
            <CopyLinkPopover text={s['id'].substring(0, 8)} value={s['id']} />
          </td>
          <td>
          ...
          <td>
            <LinkPopover
              text={Object.keys(s['packets_per_service']).length + ' services'}
              content={JSON.stringify(s['packets_per_service'])}
              placement="left"
            />
          </td>
          <td className="table-cell-action">
            <a href={'/api/pcap/sessions/' + s['id'] + '/download'}>download</a>
          </td>
        </tr>
      );
    });
    ...
    const uploadCurlCommand = createCurlCommand('/pcap/upload', 'POST', null, {
      file: '@' + (this.state.uploadSelectedFile != null && this.state.isUploadFileValid ? this.state.uploadSelectedFile.name : 'invalid.pcap'),
        ...
        <div className="pane-section pcap-list">
          <div className="section-header">
            ...
            <span className="api-response">
              <LinkPopover text={this.state.sessionsStatusCode} content={this.state.sessionsResponse} placement="left" />
            </span>
          </div>
        ...
        <div className="double-pane-container">
          <div className="pane-section">
            <div className="section-header">
              ...
              <span className="api-response">
                <LinkPopover text={this.state.uploadStatusCode} content={this.state.uploadResponse} placement="left" />
              </span>
            </div>
            ...
            <div className="section-content">
              <InputField
                type={'file'}
                name={'file'}
                invalid={!this.state.isUploadFileValid}
                active={this.state.isUploadFileFocused}
                onChange={handleUploadFileChange}
                value={this.state.uploadSelectedFile}
                placeholder={'no .pcap[ng] selected'}
              />
              <div className="upload-actions">
                <div className="upload-options">
                  <span>options:</span>
                  <CheckField name="flush_all" checked={this.state.uploadFlushAll} onChange={(v) => this.setState({uploadFlushAll: v})} />
                </div>
          ...
          <div className="pane-section">
            <div className="section-header">
              ...
              <span className="api-response">
                <LinkPopover text={this.state.processStatusCode} content={this.state.processResponse} placement="left" />
              </span>
            </div>
            ...
            <div className="section-content">
              <InputField
                ...
                invalid={!this.state.isFileValid}
                onChange={handleFileChange}
                value={this.state.fileValue}
                placeholder={'local .pcap[ng] path'}
                inline
              />

              <div className="upload-actions" style={{marginTop: '11px'}}>
                <div className="upload-options">
                  <CheckField name="flush_all" checked={this.state.processFlushAll} onChange={(v) => this.setState({processFlushAll: v})} />
                  <CheckField name="delete_original_file" checked={this.state.deleteOriginalFile} onChange={(v) => this.setState({deleteOriginalFile: v})} />
                </div>
