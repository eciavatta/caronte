...
class ConnectionsPane extends Component {
  ...
  handleConnectionsFilters = (payload) => {
    ...
    this.loadConnections({limit: this.queryLimit}, newParams).then(() => log.info('ConnectionsPane reloaded after query string update'));
  };
  ...
  handleScroll = (e) => {
    ...
    if (!this.state.loading && relativeScroll > this.scrollBottomThreashold) {
      this.loadConnections({from: this.state.lastConnection.id, limit: this.queryLimit}).then(() => log.info('Following connections loaded'));
    }
    if (!this.state.loading && relativeScroll < this.scrollTopThreashold) {
      this.loadConnections({to: this.state.firstConnection.id, limit: this.queryLimit}).then(() => log.info('Previous connections loaded'));
    ...
    if (additionalParams && additionalParams.from && !additionalParams.to) {
      if (res.length > 0) {
        ...
        if (connections.length > this.maxConnections) {
          connections = connections.slice(connections.length - this.maxConnections, connections.length - 1);
          firstConnection = connections[0];
        }
      }
    ...
    return (
      <div className="connections-container">
        {this.state.showMoreRecentButton && (
          <div className="most-recent-button">
            <ButtonField
              name="most_recent"
              variant="green"
              bordered
              onClick={() => {
                ...
                this.connectionsListRef.current.scrollTop = 0;
                this.loadConnections({limit: this.queryLimit}).then(() => {
        ...
        <div
          className={classNames('connections', {'connections-pulse': this.state.pulseConnectionsView})}
          onScroll={this.handleScroll}
          ref={this.connectionsListRef}
        >
              ...
              {this.state.connections.flatMap((c) => {
                return [
                  <Connection
                    key={c.id}
                    data={c}
                    onSelected={() => this.connectionSelected(c)}
                    selected={this.state.selected === c}
                    onMarked={(marked) => (c.marked = marked)}
                    onCommented={(comment) => (c.comment = comment)}
                    services={this.state.services}
                    selectedSimilarityProps={
                      this.state.selected && this.state.selected['port_dst'] === c['port_dst'] && generateSimilarityProps(this.state.selected)
                    }
                  />,
                  c.matched_rules.length > 0 && <ConnectionMatchedRules key={c.id + '_m'} matchedRules={c.matched_rules} rules={this.state.rules} />,
                ];
              })}
              {loading}
            </tbody>
