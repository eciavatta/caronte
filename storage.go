...
func NewMongoStorage(uri string, port int, database string, username string, password string) (*MongoStorage, error) {
	...
	opt := options.Client()
	opt.ApplyURI(fmt.Sprintf("mongodb://%s:%v", uri, port)).SetAuth(credential)
