    ...
    const accounts = Object.entries(settings.accounts)
      .map(([username, password]) => (
        <tr key={username}>
          <td>{username}</td>
          <td>
            <LinkPopover text="******" content={password} />
          </td>
          <td>
            <ButtonField variant="red" small rounded name="delete" onClick={() => this.updateParam((s) => delete s.accounts[username])} />
          </td>
        </tr>
      ))
      .concat(
        <tr key={'new_account'}>
          <td>
            <InputField value={this.state.newUsername} small active={this.state.newUsernameActive} onChange={(v) => this.setState({newUsername: v})} />
          </td>
          <td>
            <InputField value={this.state.newPassword} small active={this.state.newPasswordActive} onChange={(v) => this.setState({newPassword: v})} />
          </td>
          <td>
            <ButtonField variant="green" small rounded name="add" onClick={this.addAccount} />
          </td>
        </tr>
      );
            ...
            <div className="pane-section">
              <div className="section-header">
                ...
                <span className="api-response">
                  <LinkPopover text={this.state.setupStatusCode} content={this.state.setupResponse} placement="left" />
                </span>
              </div>
                ...
                <Container className="p-0">
                  <Row>
                    <Col>
                      <InputField
                        name="server_address"
                        value={settings.config['server_address']}
                      ...
                      <div style={{marginTop: '10px'}}>
                        <CheckField
                          checked={settings.config['auth_required']}
                          name="auth_required"
