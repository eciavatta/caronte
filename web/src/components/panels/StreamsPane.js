...
class StreamsPane extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.connection && (this.props.connection !== prevProps.connection || this.state.format !== prevState.format)) {
  ...
  loadStream = (connectionId) => {
    ...
    backend.get(`/api/streams/${connectionId}?format=${this.state.format}`).then((res) => this.setState({messages: res.json}));
  };
    ...
    switch (m.type) {
      case 'http-request': {
        let url = (
          <i>
            <u>
              <a href={'http://' + m.host + m.url} target="_blank" rel="noopener noreferrer">
                {m.host}
                {m.url}
              </a>
            </u>
    ...
    this.props.connection['matched_rules'].forEach((ruleId) => {
      ...
      rule.patterns.forEach((pattern) => {
        ...
        streamContent = reactStringReplace(streamContent, regex, (match, i) => (
          <span key={i} className="matched-occurrence" style={{backgroundColor: rule.color}}>
            {match}
          </span>
        ));
      });
      ...
      case 'http-request':
        ...
        return Object.entries(connectionMessage.metadata['reproducers']).map(([name, value]) => (
          <ButtonField
            ...
            onClick={() => {
              this.setState({
                messageActionDialog: <CopyDialog actionName={name} value={value} onHide={() => this.setState({messageActionDialog: null})} />,
              });
            }}
          />
        ));
  ...
  downloadStreamRaw = (value) => {
    if (this.state.currentId) {
      backend
        .download(`/api/streams/${this.props.connection.id}/download?format=${this.state.format}&type=${value}`)
        .then((res) => downloadBlob(res.blob, `${this.state.currentId}-${value}-${this.state.format}.txt`))
        .catch(() => log.error('Failed to download stream messages'));
    }
  };
  ...
  render() {
    ...
    let payload = content
      .filter((c) => !this.state.tryParse || (this.state.tryParse && !c['is_metadata_continuation']))
      .map((c, i) => (
        <div key={`content-${i}`} className={classNames('connection-message', c['from_client'] ? 'from-client' : 'from-server')}>
          <div className="connection-message-header container-fluid">
            <div className="row">
              <div className="connection-message-info col">
                ...
                |{' '}
                <span>
                  <strong>retransmitted</strong>: {c['is_retransmitted'] ? 'yes' : 'no'}
                </span>
              </div>
          ...
          <div className="message-content">{this.state.tryParse && this.state.format === 'default' ? this.tryParseConnectionMessage(c) : c.content}</div>
        </div>
      ));
        ...
        <div className="stream-pane-header container-fluid">
          <Row>
            <div className="header-info col">
              <span>
                <strong>flow</strong>: {conn['ip_src']}:{conn['port_src']} -&gt; {conn['ip_dst']}:{conn['port_dst']}
              </span>
              <span>
                {' '}
            ...
            <div className="header-actions col-auto">
              ...
              <ChoiceField name="view_as" inline small onlyName onChange={this.viewAs} keys={['decoded', 'raw']} values={['decoded', 'raw']} />
