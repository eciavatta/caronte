...
class RulesPane extends Component {
  ...
  loadRules = () => {
    backend
      .get('/api/rules')
      .then((res) => this.setState({rules: res.json, rulesStatusCode: res.status}))
  ...
  addRule = () => {
    if (this.validateRule(this.state.newRule)) {
      backend
        .post('/api/rules', this.state.newRule)
        .then((res) => {
          this.reset();
  ...
  render() {
    ...
    let rules = this.state.rules.map((r) => (
      ...
      >
        ...
        <td>{r['name']}</td>
        <td>
          <ButtonField name={r['color']} color={r['color']} small />
        </td>
        <td>{r['notes']}</td>
      </tr>
    ));

    let patterns = (this.state.selectedPattern == null && !isUpdate ? rule.patterns.concat(this.state.newPattern) : rule.patterns).map((p) =>
      p === pattern ? (
        <tr key={'new_pattern'}>
          <td style={{width: '500px'}}>
            <InputField
              small
              active={this.state.patternRegexFocused}
              value={pattern.regex}
              onChange={(v) => {
          ...
          <td>
            <CheckField small checked={pattern.flags['caseless']} onChange={(v) => this.updateParam(() => (pattern.flags['caseless'] = v))} />
          </td>
          <td>
            <CheckField small checked={pattern.flags['dot_all']} onChange={(v) => this.updateParam(() => (pattern.flags['dot_all'] = v))} />
          </td>
          <td>
            <CheckField small checked={pattern.flags['multi_line']} onChange={(v) => this.updateParam(() => (pattern.flags['multi_line'] = v))} />
          </td>
          <td>
            <CheckField small checked={pattern.flags['utf_8_mode']} onChange={(v) => this.updateParam(() => (pattern.flags['utf_8_mode'] = v))} />
          </td>
          <td>
            <CheckField small checked={pattern.flags['unicode_property']} onChange={(v) => this.updateParam(() => (pattern.flags['unicode_property'] = v))} />
          </td>
          <td style={{width: '70px'}}>
            <NumericField
              small
              value={pattern['min_occurrences']}
        ...
        <tr key={'new_pattern'} className="row-small">
          ...
          {!isUpdate && (
            <td>
              <ButtonField variant="blue" small rounded name="edit" onClick={() => this.editPattern(p)} />
            </td>
          )}
        </tr>
      )
    ...
    return (
      <div className="pane-container rule-pane">
          ...
          <div className="section-header">
            ...
            {this.state.rulesStatusCode && (
              <span className="api-response">
                <LinkPopover text={this.state.rulesStatusCode} content={this.state.rulesResponse} placement="left" />
              </span>
            )}
          </div>
        ...
        <div className="pane-section rule-edit">
          <div className="section-header">
            <span className="api-request">{isUpdate ? `PUT /api/rules/${this.state.selectedRule.id}` : 'POST /api/rules'}</span>
            <span className="api-response">
              <LinkPopover text={this.state.ruleStatusCode} content={this.state.ruleResponse} placement="left" />
            </span>
          </div>
          ...
          <div className="section-content">
              ...
              <Row>
                ...
                <Col style={{paddingTop: '11px'}}>
                  <NumericField
                    name="min_duration"
                    inline
                    value={rule.filter['min_duration']}
                  ...
                  <NumericField
                    name="max_duration"
                    inline
                    value={rule.filter['max_duration']}
                    error={this.state.ruleDurationError}
                    readonly={isUpdate}
            ...
            <div className="section-table">
              <Table borderless size="sm">
                ...
                <tbody>{patterns}</tbody>
              </Table>
              {this.state.rulePatternsError != null && <span className="table-error">error: {this.state.rulePatternsError}</span>}
            </div>
          </div>
          ...
          <div className="section-footer">
            ...
            <ButtonField
              variant={isUpdate ? 'blue' : 'green'}
              name={isUpdate ? 'update_rule' : 'add_rule'}
              bordered
              onClick={isUpdate ? this.updateRule : this.addRule}
            />
          </div>
        </div>
      </div>
