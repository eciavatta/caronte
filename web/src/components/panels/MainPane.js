...
class MainPane extends Component {
  ...
  componentDidMount() {
    ...
    const options = {
      strings: [
        `welcome to caronte!^1000 the current version is ${this.props.version}` +
          nl +
          'caronte is a network analyzer,^300 it is able to read pcaps and extract connections', // 0
        'the left panel lists all connections that have already been closed' +
          nl +
          'scrolling up the list will load the most recent connections,^300 downward the oldest ones', // 1
        'by selecting a connection you can view its content,^300 which will be shown in the right panel' +
          nl +
          'you can choose the display format,^300 or decide to download the connection content', // 2
        'below there is the timeline,^300 which shows the number of connections per minute per service' +
          nl +
        ...
        'at the top are the filters,^300 which can be used to select only certain types of connections' +
          nl +
          'you can choose which filters to display in the top bar from the filters window', // 5
        'in the pcaps panel it is possible to analyze new pcaps,^300 or to see the pcaps already analyzed' +
          nl +
          'you can load pcaps from your browser,^300 or process pcaps already present on the filesystem', // 6
        'in the rules panel you can see the rules already created,^300 or create new ones' +
          nl +
          'the rules inserted will be used only to label new connections, not those already analyzed' +
          nl +
          'a connection is tagged if it meets all the requirements specified by the rule', // 7
        'in the services panel you can assign new services or edit existing ones' +
          nl +
  ...
  render() {
    return (
      <div className="pane-container">
        ...
        <div className="background-pane">{this.state.backgroundPane}</div>
      </div>
    );
  }
}
