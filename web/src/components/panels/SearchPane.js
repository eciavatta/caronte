...
class SearchPane extends Component {
  ...
  performSearch = () => {
    ...
    if (this.validateSearch(options)) {
      backend
        .post('/api/searches/perform', options)
        .then((res) => {
          this.reset();
        ...
        .catch((res) => {
          this.setState({
            searchStatusCode: res.status,
            searchResponse: JSON.stringify(res.json),
            loading: false,
          });
        });
    }
  ...
  extractPattern = (options) => {
    let pattern = '';
    if (_.isEqual(options.regex_search, this.searchOptions.regex_search)) {
      // is text search
  ...
  render() {
    ...
    let searches = this.state.searches.map((s) => (
      <tr key={s.id} className="row-small row-clickable">
        ...
        <td>
          <ButtonField name="view" variant="green" small onClick={() => this.viewSearch(s.id)} />
        </td>
      </tr>
    ));
      ...
      <div className="pane-container search-pane">
          ...
          <div className="section-header">
            ...
            {this.state.searchesStatusCode && (
              <span className="api-response">
                <LinkPopover text={this.state.searchesStatusCode} content={this.state.searchesResponse} placement="left" />
              </span>
            )}
          </div>
        ...
        <div className="pane-section search-new">
          <div className="section-header">
            ...
            <span className="api-response">
              <LinkPopover text={this.state.searchStatusCode} content={this.state.searchResponse} placement="left" />
            </span>
          </div>
          ...
          <div className="section-content">
            ...
            <div className="content-row">
              <div className="text-search">
                <TagField
                  tags={(options['text_search'].terms || []).map((t) => {
                    return {name: t};
                  })}
                  name="terms"
                  min={3}
                ...
                />
                <TagField
                  tags={(options['text_search']['excluded_terms'] || []).map((t) => {
                    return {name: t};
                  })}
                  name="excluded_terms"
                  min={3}
                ...
                <InputField
                  name="exact_phrase"
                  value={options['text_search']['exact_phrase']}
                  inline
                ...
                <CheckField
                  checked={options['text_search']['case_sensitive']}
                  name="case_sensitive"
                  readonly={regexOptionsModified}
                  small
              ...
              <div className="regex-search">
                <InputField
                  name="pattern"
                  value={options['regex_search'].pattern}
                  inline
                ...
                <InputField
                  name="not_pattern"
                  value={options['regex_search']['not_pattern']}
                  inline
                ...
                <div className="checkbox-line">
                  <CheckField
                    checked={options['regex_search']['case_insensitive']}
                    name="case_insensitive"
                    readonly={textOptionsModified}
                    small
                    onChange={(v) => this.updateParam((s) => (s['regex_search']['case_insensitive'] = v))}
                  />
                  <CheckField
                    checked={options['regex_search']['multi_line']}
                    name="multi_line"
                    readonly={textOptionsModified}
                    small
                  ...
                  />
                  <CheckField
                    checked={options['regex_search']['ignore_whitespaces']}
                  ...
                  />
                  <CheckField
                    checked={options['regex_search']['dot_character']}
                    name="dot_character"
                    readonly={textOptionsModified}
                    small
          ...
          <div className="section-footer">
            <ButtonField variant="red" name="cancel" bordered disabled={this.state.loading} onClick={this.reset} />
            <ButtonField variant="green" name="perform_search" bordered disabled={this.state.loading} onClick={this.performSearch} />
          </div>
        </div>
      </div>
    );
