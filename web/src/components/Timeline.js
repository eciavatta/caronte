...
class Timeline extends Component {
  ...
  loadStatistics = async (metric) => {
    ...
    const series = new TimeSeries({
      ...
      points: zeroFilledMetrics.map((m) => [m['range_start']].concat(columns.map((c) => (metric in m && m[metric] != null ? m[metric][c] || 0 : 0)))),
    });
        ...
        <div className={classNames('time-line', {'pulse-timeline': this.state.pulseTimeline})}>
          <Resizable>
            ...
            >
              <ChartRow height={this.props.height - 70}>
                <YAxis id="axis1" hideAxisLine min={this.aggregateSeries('min')} max={this.aggregateSeries('max')} width="35" type="linear" transition={300} />
                <Charts>
                  <LineChart axis="axis1" series={this.state.series} columns={this.state.columns} style={this.createStyler()} interpolation="curveBasis" />
          ...
          <div className="metric-selection">
            <ChoiceField
              inline
              small
              keys={['connections_per_service', 'client_bytes_per_service', 'server_bytes_per_service', 'duration_per_service', 'matched_rules']}
              values={['connections_per_service', 'client_bytes_per_service', 'server_bytes_per_service', 'duration_per_service', 'matched_rules']}
              onChange={(metric) => this.loadStatistics(metric).then(() => log.debug('Statistics loaded after metric changes'))}
