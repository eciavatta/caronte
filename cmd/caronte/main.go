...
import (
	...
	"github.com/eciavatta/caronte/internal/core"
...
func main() {
	...
	storage, err := core.NewMongoStorage(*mongoHost, *mongoPort, *dbName, dbUsername, dbPassword)
	if err != nil {
	...
	applicationContext, err := core.CreateApplicationContext(storage, Version)
	if err != nil {
		log.WithError(err).WithFields(logFields).Fatal("failed to create application context")
	}

	notificationController := core.NewNotificationController(applicationContext)
	go notificationController.Run()
	applicationContext.SetNotificationController(notificationController)

	resourcesController := core.NewResourcesController(notificationController)
	go resourcesController.Run()

	applicationContext.Configure()
	applicationRouter := core.CreateApplicationRouter(applicationContext, notificationController, resourcesController)
